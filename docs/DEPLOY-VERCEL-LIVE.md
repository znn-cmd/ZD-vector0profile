...
- **Ошибка `DECODER routines::unsupported` (bootstrap или API)** — некорректный формат `GOOGLE_PRIVATE_KEY`. В `.env` ключ должен быть в одну строку, с символами `\n` (обратный слэш + n) вместо переносов строк, **без кавычек** вокруг значения:
  ```env
  GOOGLE_PRIVATE_KEY=-----BEGIN PRIVATE KEY-----\nMIIEvQIBADANB...\n...\n-----END PRIVATE KEY-----\n
  ```
  Скопируй значение поля `private_key` из JSON ключа, замени в нём каждый реальный перенос строки на два символа `\n`, и вставь в .env одной строкой. Либо положи ключ в кавычках и с реальными переносами строк (многострочно) — тогда лишние пробелы и кавычки скрипт попытается обрезать сам.
